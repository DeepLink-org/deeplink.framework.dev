...
at::Tensor format_cast(at::Tensor& tensor, diopiMemoryFormat_t target_format) {
  ...
  at::Tensor t = at::empty({0}, tensor.options());
  ::diopiTensorHandle_t out = dipu::diopi_helper::toDiopiTensorHandle(t);
...
diopiMemoryFormat_t get_format(const at::Tensor& tensor) {
  ...
  ::diopiContext context(dipu::getCurrentDIPUStream().rawstream());
  ::diopiConstTensorHandle_t input = dipu::diopi_helper::toDiopiTensorHandle(tensor);
