...
void init(void* context, int device_id, const char* config_file_path) {
  graph_runner = std::make_unique<GEGraphRunner>(context, device_id, config_file_path);
...
void add_graph(int graph_id, const char* graph_json_file,
  ...
  // aclrtStream stream;
  // int device_id = graph_runner->getDeviceId();
  // CALL_FUNC(aclrtCreateContext(&context, device_id));
  // CALL_FUNC(aclrtCreateStream(&stream));
  ...
  // CALL_FUNC(aclrtDestroyStream(stream));
  // CALL_FUNC(aclrtDestroyContext(context));
}
