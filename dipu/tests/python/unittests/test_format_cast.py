...
class TestFormatCast(TestCase):
    ...
    def test_format_cast_tensor(self):
        src_shape_format = [
            [np.float16, torch_dipu.DIOPIMemoryFormat.NCHW, (2, 2, 4, 4)],
            [np.float16, torch_dipu.DIOPIMemoryFormat.ND, (2, 2, 4, 4)]
        ]
        dst_format_list = [
        ...
        for i in src_shape_format:
            src_tensor = self.create_single_tensor(i, 1, 5)
            for dst_format in dst_format_list:
                self.check_and_get_format_tensor(src_tensor, dst_format)
